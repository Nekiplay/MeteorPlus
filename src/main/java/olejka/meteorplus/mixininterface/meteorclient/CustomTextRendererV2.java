...
public class CustomTextRendererV2 extends CustomTextRenderer implements TextRenderer {
	...
	public CustomTextRendererV2(FontFace fontFace) {
		super(fontFace);
		...
		for (int i = 0; i < fonts.length; i++) {
			((Buffer) buffer).flip();
			fonts[i] = new FontV2(buffer, (int) Math.round(18 * ((i * 0.5) + 1)));
		}
	}
	...
	public double getWidth(String text, int length, boolean shadow) {
		...
		FontV2 font = building ? this.font : fonts[0];
